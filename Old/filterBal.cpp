...
int balance(long m,unsigned int *i,unsigned int *j,float *x, double *b, double *report,int *allIters, double tol, int maxiter, double del, int *totIter, int threads, unsigned int k, int width, double *perc);
...
{
  fprintf(stderr, "Usage: %s [-e (oe)][-p percent][-P delta_p][-v verbose][-t tol][-I max_iterations][-d delta][-A All_iterations][-T threads] <hicfile> <chromosome> <resolution> <outfile>\n", argv0);
...
int main(int argc, char *argv[]) {
	...
	if (width <2) width = 2;
	time(&t0);
	iter = balance(m,ii,jj,xx, b, report,allIters, tol, maxiter, del, &totalIt, threads, k, width, &perc);
	time(&t1);
