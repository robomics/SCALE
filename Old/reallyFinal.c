...
//	find the number of nonzeros in each row
	...
	double *diag = (double *) malloc(k*sizeof(double));
	double *diag0 = (double *) malloc(k*sizeof(double));
	...
	if (width > 0) {
		for (p=0;p<k;p++) diag[p] = 0;
        	...
        	qsort(diag0,nd0,sizeof(double),cmpfunc);
        	double med = diag0[(int) nd0/2];
		lowMed = (int) med*0.01;
	}
	else {
		for (p=0;p<k;p++) diag[p] = 1;
		lowMed = 1;
	}
...
//	find rows which are identically 0 and the perc proportion of rows with the lowest row sums and exclude them
	...
	for (p=0;p<k;p++) if (diag[p] < lowMed) bad[p] = 1;
...
//  just halve low
			...
			for (p=0;p<k;p++) if (diag[p] < lowMed) {
				bad[p] = 1;
				one[p] = 0;
			}
                        iter = 0;
