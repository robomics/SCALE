...
int bal(long m,int *i,int *j,real *x, real *d, real report[][2],int verb, double tol,double *pppp, int maxiter, int zerodiag, real del);

static void usage(const char *argv0)
{
  fprintf(stderr, "Usage: %s [-m memarray][-p percent][-v verbose][-s silent][-t tol][-d del][-I max_iterations][-z remove_zero_diag] <infile> <outfile>\n", argv0);
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "m:p:v:s:t:d:I:z:h")) != -1) {
    switch (opt) {
    case 'm':
      m1 = (long) atof(optarg);
      break;
  ...
  int cont=0;
  ftime(&t1);
  iter = bal(m, i, j, x, d, report, 1-silent, tol,&perc, maxiter, zerodiag, del);
  ...
  printf("final p is %g; final error in vector is %g and in row sums is %g\n",perc,report[maxiter+1][0],report[maxiter+1][1]);
