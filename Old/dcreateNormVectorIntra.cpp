...
int scale(long m,int *i,int *j,float *x, double *z,double *b, double *report, int *iters, double tol,double *pppp,double *pppp1, int maxiter, int zerodiag, double del, double dp, double dp1, int *totalIt, int threads, int k);
...
//	loop accross all chromosomes
	...
	fclose(fout);
	ftime(&end);
	if (verb) printf("\n**************    all togeter took %15.10lf seconds\n",((double) (end.time - start.time)) + 0.001*(end.millitm - start.millitm));
	return(iter0);
}
