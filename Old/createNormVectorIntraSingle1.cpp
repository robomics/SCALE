...
{
  fprintf(stderr, "Usage: %s [-p percent][-P delta_p][-q percent1][-Q delta_q][-v verbose][-t tol][-I max_iterations][-z remove_zero_diag][-d delta][-A All_iterations][-T threads] <hicfile> <chromosome> <resolution> <outfile> <[vector_file]>\n", argv0);
...
int main(int argc, char *argv[]) {
	...
	ftime(&t0);
	k = getMatrix(fname, binsize, norm, &ii, &jj, &xx, &m, chr);
	ftime(&t1);
	if (k < 0) {
