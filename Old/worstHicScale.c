...
int scale(long m,int *i,int *j,float *x, float *z,float *b, float *report,int *allIters, float tol,float *pppp,float *pppp1, int maxiter, int zerodiag, float del, float dp, float dp1,int *totIter, int threads, int k)
{  
	long p;
	int n0;
...
//	if less than 5 iterations (so less than 5 errors) and less than 2 row sums errors, there is nothing to chek
            	if ( (nerr < 2 || (nerr >= 2 && errors[nerr-1] < 0.5*errors[nerr-2])))  continue;
//	otherwise check
                if (iter > 5) {
