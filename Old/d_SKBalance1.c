...
printf("ber = %g; iter = %d; perc = %g; low = %g\n",ber,iter,perc,low);
       		if (ber < tol) {
			if (perc == 0) break;
                        conv = true;
                        conv_perc = perc;
			...
			for (p=0;p<k;p++) dr[p] = 1.0;
			for (p=0;p<k;p++) dc[p] = 1.0;
			continue;
		}

//	otherwise check
                if (iter <= 5) continue;
printf("iter = %d; all_iters = %d; x6 = %g; x1 = %g\n",iter,all_iters, report[all_iters-6],report[all_iters-1]);
