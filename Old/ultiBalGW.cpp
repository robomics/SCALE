...
{
  fprintf(stderr, "Usage: %s [-i (inter_only)][-l low][-v verbose][-t tol][-I max_iterations][-d delta][-A All_iterations][-T threads] <hicfile> <resolution> <outfile>\n", argv0);
...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "il:v:t:d:I:A:T:h")) != -1) {
		switch (opt) {
				case 'i':
					interOnly = true;
					break;
...
//	if (verb) printf("iterations took %15.10lf seconds\n",((double) (t1.time - t0.time)) + 0.001*(t1.millitm - t0.millitm));
	...
	if (verb) for (p=0;p<totalIt;p++) printf("%d: %30.15lf\n",allIters[p],report[p]);
	printf("total %d iterations; final low is %d; final perc = %g\n",totalIt,low,perc);
