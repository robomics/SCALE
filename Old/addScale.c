...
int scale(long m,int *i,int *j,float *x, float *z,float *b, float *report,int *allIters, float tol,float *pppp, int maxiter, float del, float dp, int *totIter, int threads, int k)
{  
	long p;
	int n0;
...
//	row is the current rows sum; dr and dc are the current rows and columns scaling vectors
	...
	int iter=0;
	for (p=0;p<k;p++) current[p] = sqrt((double) (dr[p]*dc[p]));
...
//	calculate current scaling vector
		for (p=0;p<k;p++) b[p] = sqrt((double) (dr[p]*dc[p]));
int w1=0,w2=0;
for (p=0;p<k;p++) {
	if (isnan(dr[p])) w1++;
	if (isnan(dc[p])) w2++;
