...
{
  fprintf(stderr, "Usage: %s [-m memarray][-p percent][-v verbose][-s silent][-t tol][-d del][-I max_iterations] <infile> <outfile>\n", argv0);
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "m:p:v:s:t:d:I:h")) != -1) {
    switch (opt) {
    case 'm':
      m1 = (long) atof(optarg);
      break;
  ...
  int cont=0;
  ftime(&t1);
  iter = bal(m, i, j, x, d, report, 1-silent, tol,&perc, maxiter, del);
