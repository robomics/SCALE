...
int main(int argc, char *argv[]) {
  ...
  int cont=0;
  ftime(&t1);
  iter = bal(m, i, j, x, d, report, 1-silent, tol,perc, maxiter, zerodiag, cont);
  ...
  while (true) {
	  ...
	  printf("Enter output_file_name new_p zero_diag max_it cont\n");
	  junk = scanf("%s %lg %d %d %d",ans,&perc,&zerodiag,&maxiter,&cont);
	  ftime(&t1);
  	  iter = bal(m, i, j, x, d, report, 1-silent, tol,perc, maxiter, zerodiag, cont);
	  ftime(&end);
  	  printf("iterations took %15.10lf seconds\n",((real) (end.time - t1.time)) + 0.001*(end.millitm - t1.millitm) );
