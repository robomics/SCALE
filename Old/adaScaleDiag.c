...
int balance(long m,unsigned int *i,unsigned int *j,float *x, double *b, double *report,int *allIters, double tol,double *pppp, int maxiter, double del, int *totIter, int threads, unsigned int k, int width)
{  
	long p;
...
//      New percentage policy   
        double conv_ber, conv_perc, div_perc;
...
FILE *diagout = fopen("diag.out","w");
fprintf(diagout,"%d %d\n",k,n0);
for (p=0;p<n0;p++) fprintf(diagout,"%g\n",r0[p]);
        qsort(r0,n0,sizeof(double),cmpfunc);
	double med = r0[(int) n0/2];
	double maxVal = r0[(int) (maxPerc*n0)];
	maxPerc = maxVal/med;
        low = med*perc;
printf("med = %g; maxPerc = %g; maxVal = %g; low = %g\n",med,maxPerc,maxVal,low);
...
printf("div: %g %g %d %d\n",perc,low,iter,all_iters);
		low_div = low;
			...
			if (low_conv*0.9 < low_div) {
				...
				for (p=0;p<k;p++) if (nz[p] < low_conv) {
					bad[p] = 1;
					one[p] = 0;
				}
				break;
