...
int scale(long m,int *i,int *j,real *x, real *z,real *b, real *report, int verb,real tol,real perc,real perc1, int maxiter,real del,int trials,int zerodiag, real *dr, real *dc, int cont);

static void usage(const char *argv0)
{
  fprintf(stderr, "Usage: %s [-m memarray][-p percent][-q percent1][-v verbose][-s silent][-t tol][-d delta][-f trials][-a attempts][-I max_iterations][-z remove_zero_diag] <infile> <vecor_file> <outfile>\n", argv0);
...
int main(int argc, char *argv[]) {
  ...
  int cont=0;
  ftime(&t1);
  iter = scale(m,i,j,x, z,b,report,1-silent,tol,perc,perc1,maxiter,delta,trials,zerodiag,dr,dc,cont);
  ...
  while (true) {
	  ...
	  printf("Enter output_file_name new_p new_q max_it cont\n");
          for (p=0;p<k;p++) z[p] = z0[p];
	  scanf("%s %g %g %d %d",ans,&perc,&perc1,&maxiter,&cont);
	  ftime(&t1);
	  iter = scale(m,i,j,x, z,b,report,1,tol,perc,perc1,maxiter,delta,trials,zerodiag,dr,dc,cont);
	  ftime(&end);
  	  printf("iterations took %15.10lf seconds\n",((real) (end.time - t1.time)) + 0.001*(end.millitm - t1.millitm) );
  	  fout = fopen(ans,"w");
  	  for (p=0;p<n;p++) fprintf(fout,"%30.15lf\n",b[p]);
