...
int scale(long m,int *i,int *j,float *x, float *z,float *b, float *report, int *iters, float tol,float *pppp,float *pppp1, int maxiter, int zerodiag, float del, float dp, float dp1, int *totalIt);

static void usage(const char *argv0)
{
  fprintf(stderr, "Usage: %s [-m memarray][-p percent][-P delta_p][-q percent1]i[-Q delta_q][-v verbose][-t tol][-I max_iterations][-z remove_zero_diag][-d delta][-A All_iterations] <infile> <vecor_file> <outfile>\n", argv0);
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "m:p:P:q:Q:v:t:d:I:z:A:h")) != -1) {
    switch (opt) {
    case 'm':
      m1 = (long) atof(optarg);
      break;
  ...
  ftime(&t1);
  iter = scale(m,i,j,x, z,b, report,allIters, tol,&perc,&perc1, maxiter, zerodiag, del, dp, dp1, &totalIt);
