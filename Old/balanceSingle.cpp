...
int balance(long m,unsigned int *i,unsigned int *j,float *x, float *b, float *report,int *allIters, float tol,float *pppp, int maxiter, float del, float dp, int *totIter, int threads, unsigned int k);
...
{
  fprintf(stderr, "Usage: %s [-o oe][-p percent][-P delta_p][-v verbose][-t tol][-I max_iterations][-d delta][-A All_iterations][-T threads] <hicfile> <chromosome> <resolution> <outfile>\n", argv0);
...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "o:p:P:v:t:d:I:A:T:h")) != -1) {
		switch (opt) {
				case 'o':
